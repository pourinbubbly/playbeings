          ...
          <div className="px-8 pb-8 relative">
            <div className="flex items-end justify-between -mt-20 mb-4">
              <div className="flex items-end gap-4">
                {/* Avatar */}
                <div className="w-32 h-32 rounded-full border-4 border-background bg-background overflow-hidden flex-shrink-0 shadow-2xl">
                  {targetUser.avatar ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {sortedGames.map((game) => (
                <div key={game._id} className="glass-card border border-[var(--neon-magenta)]/20 overflow-hidden hover:border-[var(--neon-magenta)]/40 transition-all">
                  <div className="w-full aspect-[616/353] bg-black/40 relative flex items-center justify-center">
                    {game.imageUrl ? (
                      <img 
                        src={game.imageUrl} 
                        alt={game.name}
                        ...
                        onError={(e) => {
                          ...
                          if (parent) {
                            ...
                            icon.className = "flex items-center justify-center w-full h-full absolute inset-0";
                            icon.innerHTML = '<svg class="w-12 h-12 text-[var(--neon-magenta)] opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>';
                            parent.appendChild(icon);
                          }
                        }}
                      />
                    ) : (
                      <Gamepad2 className="w-12 h-12 text-[var(--neon-magenta)] opacity-50" />
                    )}
                  </div>
                  <div className="p-3">
